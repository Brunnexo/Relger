...
public class Atributos
{
	...
	private boolean tMec;
	private boolean tProg;
	private boolean tEng;
	private boolean tProj;
	private boolean tAdm;

	private String descTrabalho;

	private boolean hExtraProgramada, hExtraRegistro = true;

	private boolean hExtra;
	private boolean bHoras;
	...
	public String dataCondicional()
	{
		...
		try {
			ResultSet rs = Extra.retornar(this.crachaFunc);
			while (rs.next())
			{
	...
	public boolean horaExtra()
	{
		...
		if (this.override | this.hExtraMensalista)
			this.hExtra = true;
		else if (fimDeSemana)
			this.hExtra = true;
		else if (this.isHorista() && this.tempoTrabalhado >= this.tempo())
			this.hExtra = true;
		else if (this.isMensalista() && this.tempoTrabalhado >= (this.tempo() + 72))
			this.hExtra = true;
		else
			this.hExtra = false;
	...
	public int tempoRestante()
	{
		...
		else if (this.hExtraMensalista)
			tempoRestante = (60 - this.tempoExtraTrabalhado);
		else if (this.isMensalista() && (this.tempoTrabalhado >= (this.tempo() + 72)))
			tempoRestante = (60 - this.tempoExtraTrabalhado);
		else if (this.isMensalista() && (this.tempoTrabalhado >= this.tempo()) && (this.tempoTrabalhado <= (this.tempo() + 72)))
			tempoRestante = ((this.tempo() + 72) - this.tempoTrabalhado);
		else if (this.isHorista() && (this.tempoTrabalhado >= this.tempo()))
			tempoRestante = (60 - this.tempoExtraTrabalhado);
